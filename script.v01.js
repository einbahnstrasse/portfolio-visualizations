(function() {
    ...
    // Retrieve the audio URL from the HTML element
    var audioElement = document.getElementById('audio-container');
    AUDIO_URL = audioElement.getAttribute('data-audio-url');
    ...
    params = {
      ...
      volume: 1,
      progress: 0,
      fullscreen: false // Add fullscreen parameter
    };
    ...
    play = function() {
      ...
      gui.__controllers.find(controller => controller.property === 'isPlaying').updateDisplay();
    };
  
    initGUI = function() {
      var modeController, sizeController, themeController, playController, volumeController, progressController, fullscreenController; // Add fullscreenController variable
      gui = new dat.GUI();
      // if window.innerWidth < 500
      gui.close();

      // Add fullscreen toggle controller
      fullscreenController = gui.add(params, 'fullscreen').name('Fullscreen');
      fullscreenController.onChange(function(value) {
        toggleFullscreen(value);
      });

      playController = gui.add(params, 'isPlaying').name('Play/Pause');
      ...
      volumeController = gui.add(params, 'volume', 0, 1).name('Volume');
      ...
      progressController = gui.add(params, 'progress', 0, 1).name('Progress').listen();
    ...
    initAudio = function() {
      ...
      audio.addEventListener('timeupdate', function() {
        if (audio.duration) {
          params.progress = audio.currentTime / audio.duration;
          gui.__controllers.find(controller => controller.property === 'progress').updateDisplay();
        }
      });
